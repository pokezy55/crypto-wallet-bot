...
// All ERC20 addresses below are lowercase (not EIP-55, for experiment only)
export const CHAINS = {
  base: {
    chainId: 8453,
    name: "Base",
...
// For each token in each chain, if token.address exists, convert to lowercase
Object.values(CHAINS).forEach(chain => {
  chain.tokens.forEach(token => {
    if (token.address) token.address = token.address.toLowerCase();
  });
});
