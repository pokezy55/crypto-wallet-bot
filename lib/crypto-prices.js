...
export async function fetchPrices(tokens) {
  ...
  if (!ids) return Object.fromEntries(tokens.map(t => [t.symbol, 0]));
  try {
    ...
    for (const t of tokens) {
      if (t.priceId && data[t.priceId] && data[t.priceId].usd != null) {
        prices[t.symbol] = data[t.priceId].usd;
      } else {
        prices[t.symbol] = lastPrices[t.symbol] || 0;
        if (!data[t.priceId]) missing.push(t.symbol);
      }
    }
    if (missing.length > 0) {
      console.warn('Missing price for tokens:', missing.join(', '));
    }
    lastPrices = prices;
    return prices;
  } catch (e) {
    ...
    const fallback = {};
    for (const t of tokens) {
      fallback[t.symbol] = lastPrices[t.symbol] || 0;
    }
    return fallback;
  }
} 