...
export function SettingsProvider({ children }) {
    ...
    const loadSettings = () => {
      try {
        ...
        if (savedSettings) {
          ...
          // Periksa apakah harus menampilkan lock screen
          if (parsedSettings.lockOnLoad && parsedSettings.pinEnabled) {
            setIsLocked(true);
          } else {
            // Jika tidak perlu lock, langsung set unlocked
            setIsUnlocked(true);
          }
        } else {
          // Jika tidak ada settings, langsung set unlocked
          setIsUnlocked(true);
        }
      } catch (error) {
        console.error('Error loading settings from localStorage:', error);
        // Jika error, tetap set unlocked untuk menghindari blocking
        setIsUnlocked(true);
      } finally {
        setIsLoading(false);
      }
  ...
  const updateSettings = (newSettings) => {
    ...
    // Jika lockOnLoad dimatikan, pastikan tidak terkunci saat reload
    if (newSettings.hasOwnProperty('lockOnLoad') && newSettings.lockOnLoad === false) {
      setIsLocked(false);
    }
  };
  ...
  const verifyPin = async (userId, pin) => {
    try {
      ...
      if (data.success) {
        // Jika PIN valid dan aplikasi terkunci, buka kunci
        if (isLocked) {
          setIsLocked(false);
          setIsUnlocked(true);
        }
  ...
  // Fungsi untuk membuka kunci aplikasi
  const unlockApp = () => {
    setIsLocked(false);
    setIsUnlocked(true);
  };
