...
// Helper to safely normalize address or return null
function safeGetAddress(addr) {
  try {
    return ethers.getAddress(addr);
  } catch (e) {
    console.warn('Invalid ERC20 address (checksum error):', addr, e.message);
    return null;
  }
}
...
//   if (!checksumAddress) continue; // skip invalid
//   // Optionally verify contract exists
//   const code = await provider.getCode(checksumAddress);
//   if (!code || code === '0x') { console.warn('No contract at address:', checksumAddress); continue; }
//   ...
// }
