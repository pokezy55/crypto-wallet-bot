...
async function getUserReferrals(userId) {
  try {
    ...
    try {
      ...
      // Format data untuk menampilkan nama pengguna dengan lebih baik
      return rows.map(row => ({
        ...row,
        username: row.username || row.first_name || `User ${row.telegram_id?.toString().substring(0, 5)}` || 'Anonymous',
        display_name: row.first_name ? (row.last_name ? `${row.first_name} ${row.last_name}` : row.first_name) : (row.username || 'Anonymous')
      }));
    } catch (error) {
