...
async function getUserReferrals(userId) {
  try {
    ...
    try {
      ...
      // Format data untuk menampilkan username dari kode referral
      return rows.map(row => ({
        ...row,
        username: row.custom_code || `User${row.telegram_id?.toString().substring(0, 5) || row.id?.toString().substring(0, 5) || ''}`,
        display_name: row.custom_code || `User${row.telegram_id?.toString().substring(0, 5) || row.id?.toString().substring(0, 5) || ''}`
      }));
    } catch (error) {
