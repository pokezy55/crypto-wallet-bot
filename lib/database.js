...
async function getSwapClaims() {
  const db = await connectDB();
  const { rows } = await db.query('SELECT * FROM claims WHERE type = $1', ['swap']);
  return rows;
}
...
async function approveSwapClaim(claimId) {
  const db = await connectDB();
  await db.query('UPDATE claims SET status = $1 WHERE id = $2', ['completed', claimId]);
}

// Get all referral claims
async function getReferralClaims() {
  const db = await connectDB();
  const { rows } = await db.query('SELECT * FROM claims WHERE type = $1', ['referral']);
  return rows;
}
...
async function claimTaskReward(userId, taskId) {
  const db = await connectDB();
  await db.query('UPDATE tasks SET claimed = TRUE WHERE id = $1 AND user_id = $2', [taskId, userId]);
}

// Get user referral data
async function getUserReferralData(userId) {
  const db = await connectDB();
  const stats = await db.query('SELECT COUNT(*) AS total, SUM(earned) AS earned FROM referrals WHERE user_id = $1', [userId]);
  const referrals = await db.query('SELECT * FROM referrals WHERE user_id = $1', [userId]);
...
async function claimReferralEarning(userId) {
  const db = await connectDB();
  await db.query('UPDATE referrals SET claimed = TRUE WHERE user_id = $1 AND claimed = FALSE', [userId]);
}
...
async function getStats() {
  ...
  const totalClaims = await db.query('SELECT COUNT(*) FROM claims');
  return {
...
async function getUserSeedPhrase(userId) {
  const db = await connectDB();
  const { rows } = await db.query('SELECT seed_phrase_encrypted FROM wallets WHERE user_id = $1', [userId]);
