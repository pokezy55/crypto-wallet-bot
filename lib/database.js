...
// Update swap progress (upsert)
async function updateSwapProgress(userId, totalSwapUSD, status) {
  await pool.query(`
    INSERT INTO campaign_progress (user_id, task_type, total_swap_usd, status, updated_at)
    VALUES ($1, 'swap', $2, $3, NOW())
    ON CONFLICT (user_id, task_type)
    DO UPDATE SET total_swap_usd = $2, status = $3, updated_at = NOW()
  `, [userId, totalSwapUSD, status]);
}

// Get last swap progress
async function getLastSwapProgress(userId) {
  const { rows } = await pool.query(`
    SELECT * FROM campaign_progress WHERE user_id = $1 AND task_type = 'swap'`, [userId]);
  return rows[0] || null;
}

// Add XP to user_xp table
async function addUserXP(userId, xp) {
  await pool.query(`
    INSERT INTO user_xp (user_id, xp, updated_at)
    VALUES ($1, $2, NOW())
    ON CONFLICT (user_id)
    DO UPDATE SET xp = user_xp.xp + $2, updated_at = NOW()
  `, [userId, xp]);
  // Optionally, update user level here if needed
}

// Update user level berdasarkan XP
async function updateUserLevel(userId) {
  const { rows } = await pool.query('SELECT xp FROM user_xp WHERE user_id = $1', [userId]);
...
module.exports = {
  ...
  setUserCustomCode,
  getUserByCustomCode,
  getAllUserWalletsReal,
  updateDepositProgress,
  getLastDepositProgress,
  updateSwapProgress,
  getLastSwapProgress,
  addUserXP,
  updateUserLevel
