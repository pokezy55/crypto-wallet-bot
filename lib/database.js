...
async function getUserReferrals(userId) {
  console.log('getUserReferrals called with userId:', userId);
  try {
    ...
    try {
      ...
      // Format data untuk menampilkan username telegram
      const formattedRows = rows.map(row => ({
        ...row,
        // Simpan username asli jika ada
        original_username: row.username,
        // Gunakan telegram_id sebagai username jika username tidak ada
        username: row.username || `${row.telegram_id?.toString().substring(0, 5) || row.id?.toString().substring(0, 5) || ''}`,
        // Tampilkan username dengan format @username
        display_name: row.username || `${row.telegram_id?.toString().substring(0, 5) || row.id?.toString().substring(0, 5) || ''}`
      }));
      
      console.log('Formatted referrals:', formattedRows.map(row => ({ username: row.username, display_name: row.display_name })));
      ...
      if (error.code === '42703') { // PostgreSQL error code untuk kolom tidak ada
        ...
        console.log('Formatted referrals (alt):', formattedRows.map(row => ({ username: row.username, display_name: row.display_name })));
