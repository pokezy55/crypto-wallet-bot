...
async function getWalletByUserId(userId) {
  try {
    ...
    if (!rows[0].seed_phrase) {
      ...
      // Return wallet without seed phrase instead of throwing error
      return {
        ...rows[0],
        seed_phrase: ''
      };
...
async function getUserReferrals(userId) {
  const { rows } = await pool.query(`
    SELECT r.*, u.username, u.first_name, w.address,
      ...
      (SELECT status FROM claims WHERE user_id = $1 AND type = 'referral' LIMIT 1) as reward_status
    FROM referrals r 
