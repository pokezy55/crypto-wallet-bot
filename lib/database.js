  ...
  try {
    ...
    try {
      ...
      const formattedRows = rows.map(row => ({
        ...row,
        username: row.username || `User${row.telegram_id?.toString().substring(0, 5) || row.id?.toString().substring(0, 5) || ''}`,
        display_name: row.username ? `@${row.username}` : (row.custom_code || `User${row.telegram_id?.toString().substring(0, 5) || row.id?.toString().substring(0, 5) || ''}`)
      }));
