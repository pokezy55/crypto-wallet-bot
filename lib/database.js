...
// Hitung total swap USD untuk wallet tertentu dari tabel transactions
async function getTotalSwapUSDByWalletId(walletId) {
  const { rows } = await pool.query(
    `SELECT COALESCE(SUM(usd_value), 0) AS total_swap FROM transactions WHERE wallet_id = $1 AND tx_type = 'swap'`,
    [walletId]
  );
