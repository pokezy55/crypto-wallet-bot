  ...
  try {
    ...
    try {
      ...
      const formattedRows = rows.map(row => {
        ...
        return {
          ...row,
          // Pastikan username Telegram asli digunakan jika ada
          username: row.username || '',
          // Tampilkan username Telegram dengan format @username jika ada, jika tidak gunakan ID
          display_name: row.username || row.telegram_id?.toString() || row.id?.toString()
        };
      });
