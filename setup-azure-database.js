...
const client = new Client({
  host: process.env.AZURE_POSTGRES_HOST || 'crypto-wallet-bot-db.postgres.database.azure.com',
  port: 5432,
  database: process.env.AZURE_POSTGRES_DATABASE || 'crypto_wallet_bot',
  user: process.env.AZURE_POSTGRES_USER || 'postgres',
  password: '@Bandung123',
  ssl: {
...
async function setupAzureDatabase() {
  try {
    ...
    console.log('\n📊 Azure Database schema created successfully!');
    ...
    await client.end();
    console.log('\n👋 Azure Database setup completed!');

  } catch (error) {
    console.error('❌ Azure Database setup failed:', error.message);
